    ...
    public function index(User $user ,Request $request)
    {
        if(Request()->ajax()) {
            ...
            ->addColumn('country', function($row) {
                if($row->finished_profile > 0){
                    return '<span>'.$row->country['name'].'</span><img class="smallflag" src="'.str_replace('{iso}',$row->country['iso'] , $row->country['flag']).'" alt="flag" width="25"/>';
                }else{
                    return '';
                }
            ...
            ->rawColumns(['avatar','status','country'])
            ->make(true);
        }
